...
- [ ] **Step 4: 跑測試確認通過** → `uv run pytest tests/test_models.py -v` → PASS
- [ ] **Step 5: Commit** → `git add -A && git commit -m "feat: Holding/Change value objects"`

---

### Task 4: validate — 三道驗證（spec §5）
...
- [ ] **Step 2: 確認失敗** → `uv run pytest tests/test_validate.py -v` → FAIL
- [ ] **Step 3: 實作**
...
- [ ] **Step 4: 確認通過** → `uv run pytest tests/test_validate.py -v` → PASS
- [ ] **Step 5: Commit** → `git commit -am "feat: 入庫前三道驗證"`

---

### Task 5: diff — 異動偵測（spec §5 雙門檻）
...
- [ ] **Step 2: 確認失敗** → `uv run pytest tests/test_diff.py -v` → FAIL
- [ ] **Step 3: 實作**
...
- [ ] **Step 4: 確認通過** → PASS
- [ ] **Step 5: Commit** → `git commit -am "feat: 快照 diff 異動偵測（雙門檻）"`

---

### Task 6: finmind client

**Files:** Create: `scraper/src/activeetf/finmind.py`, Test: `scraper/tests/test_finmind.py`

- [ ] **Step 1: 失敗測試（responses 模擬 HTTP）**
...
- [ ] **Step 6: Commit** → `git commit -am "feat: FinMind 行情 client"`
...
- [ ] **Step 4: 本機跑一次** → `uv run python scripts/probe.py` → 記下各家狀態
- [ ] **Step 5: Commit** → `git commit -am "feat: ETF registry 與官網探測腳本"`
...
- [ ] **Step 2: Commit** → `git commit -am "feat: adapter 統一介面"`

---

### Task 10: 首個 adapter — 群益（範本）
...
- [ ] **Step 2: 對 fixture 寫不變量測試（不 hardcode 具體持股，資料每天會變）**
...
- [ ] **Step 3: 確認失敗** → `uv run pytest tests/test_adapter_capital.py -v` → FAIL
- [ ] **Step 4: 實作解析（依 fixture 實際結構調整 selector——以下為 HTML 表格版的起點）**
...
- [ ] **Step 5: 確認通過** → PASS（依 fixture 實況修 selector 直到過）
- [ ] **Step 6: 真連線煙霧測試（人工）**
...
- [ ] **Step 7: 補 00982A / 00997A 的 pcf_url 到 registry（同站不同基金 id，觀察 URL 規律），各跑一次 Step 6 驗證**
- [ ] **Step 8: Commit** → `git commit -am "feat: 群益 adapter（含 fixture 測試）"`

---

### Task 11: pipeline — 每日主流程

**Files:** Create: `scraper/src/activeetf/pipeline.py`, Test: `scraper/tests/test_pipeline.py`

- [ ] **Step 1: 失敗測試（全用 fake，不碰網路與 DB）**
...
- [ ] **Step 4: 確認通過** → `uv run pytest tests/test_pipeline.py -v` → PASS
- [ ] **Step 5: Commit** → `git commit -am "feat: 每日 pipeline（隔離、冪等、驗證整合）"`

---

### Task 12: 其餘投信 adapter（逐家循環）
...
### Task 13: metrics — 報酬與擇時勝率（spec §6）
