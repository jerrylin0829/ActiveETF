...
- ⚠️ **2026-07-31 追加（實測後）**：六支的「請求日」不全等於「資料日」——國泰、中信為 0 位移，統一／第一金／安聯為前一交易日，元大為次一交易日。`fetch_at` 改回 `(holdings, source_date)`，請求日以**交易日**位移換算，寫入前核對資料日等於**逐 ETF 算出的期望資料日**（多數檔等於 `trade_date`，00988A 為 `T-1`）。設計見 spec §3.1，**此 gate 不可省略**
