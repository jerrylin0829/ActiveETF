# Generator handoff — 爬蟲韌性：pipeline 重試、非 JSON 回應可辨識、缺口補抓策略
...
2026-07-25 由 Planner 實測：`POST /API/ETFAPI/api/Fund/GetFundTradeInfoDate` 對 00985A 與 00980A 皆回 HTTP 200 但 body 是 17KB 的 PNG。**以瀏覽器開啟該圖確認內容為野村官網的維護公告**：「野村投資理財網進行電腦機房主機例行性維護，所有野村投資理財網站（含網路交易/語音服務/客服專線）將暫停運作」。故 `JSONDecodeError` 的成因是**上游維護期間以圖片回應所有請求**，非機器人阻擋、亦非解析邏輯錯誤。00985A 於 07-24 單獨失敗係維護時段與抓取時序交錯所致。

> Planner 自我修正：初判為「WAF 阻擋」係僅憑 `Server: BigIP` 與二進位回應推論，未檢視圖片內容。此為以推論取代觀察的錯誤，記錄備查。

三個結構性問題因此曝光：

1. `pipeline.py` **沒有任何重試**，`adapters/base.py` 卻明文寫「重試、驗證、入庫都在 pipeline，adapter 保持純粹」——文件承諾未實作。單次網路打嗝即整檔當日失敗；野村每檔要打兩次 API，機率加倍。
2. 上游回傳非 JSON（維護頁、錯誤頁、阻擋頁皆可能）時，錯誤訊息是難懂的 `JSONDecodeError`，看不出真正原因。
...
- ✅ 新增可辨識的例外型別，讓 `scrape_log` 直接看得出「上游回的不是 JSON、長什麼樣」而非埋在 traceback
...
- 可重試的例外：`requests.Timeout`、`requests.ConnectionError`、新增的 `UpstreamNotJsonError`（見 Scope 2）
- 不可重試：`ValidationError`、`KeyError`／`IndexError`（解析結構變了，重試無意義）
- 最終失敗才寫 `scrape_log`，錯誤訊息需含「已重試 N 次」
- 重試不得繞過既有的 `scraped_ok` 冪等檢查

### 2. 非 JSON 回應可辨識（`scraper/src/activeetf/adapters/base.py` + `nomura.py`）

- 於 `base.py` 新增 `UpstreamNotJsonError(RuntimeError)` 與共用 helper，例如 `ensure_json(response) -> dict`：當 `content-type` 非 JSON、或 body 前綴為已知二進位魔術數字（PNG `\x89PNG`、JPEG `\xff\xd8`、PDF `%PDF`）、或 `response.json()` 拋錯時，改拋 `UpstreamNotJsonError`，訊息含 HTTP 狀態、content-type、body 長度與前 40 bytes 的 repr。**命名刻意不預設成因**——非 JSON 可能是維護頁、錯誤頁或阻擋，實際原因需人工檢視（本次即為維護公告圖片）
...
- `nomura.fetch()` 永遠取 API 回的 `LatestDate`；`_request_body` 雖有 `date` 參數但未被用於歷史查詢
- **2026-07-25 更新：此節的前提已被推翻**——探測證實 12 檔 ETF（統一/元大/國泰/第一金/安聯/中信）的上游 API 支援指定歷史日期，詳見 `docs/superpowers/specs/2026-07-25-historical-backfill-design.md`。方向 B 已由該 spec 採納並展開設計

**兩個方向，請 User 擇一**：

- **方向 B（已於 2026-07-25 由歷史回補 spec 採納）**：為支援歷史查詢的 adapter 引入可選能力 `fetch_at(entry, date)`。**本 handoff 的 Scope 3 因此收斂為：等歷史回補片落地後，pipeline 是否要對這些 adapter 自動回看 3 天補缺口**——此為獨立的小決策，待 User 於歷史回補完成後裁決。
- **對不支援歷史查詢的 13 檔**：跨交易日即永久缺口，由 Dashboard 黃條如實揭露（此部分無爭議）。

裁決前，本 PR **只做 Scope 1、2**；Scope 3 待裁決後另開 PR 或補進本 PR。

## Non-goals

- 不繞過、不規避上游的機器人偵測機制。（註：野村本次為官網維護，非阻擋；重試退避即為適當處置）
...
- 非 JSON 辨識測試涵蓋：回傳 PNG bytes → 拋 `UpstreamNotJsonError` 且訊息含 content-type 與 body 前綴；正常 JSON → 正常回傳
- 重試不影響既有冪等性（同日已 ok 者不重跑）
- 每日請求量仍符合禮貌原則（重試僅在失敗時發生，最壞情況每檔 3 次）

## Required Verification

- 單元測試以 mock/fake 覆蓋，**不對野村官網發測試請求**（官網維護中，且避免加重上游負擔）
- 無 DB 寫入變更，不需整合測試證據
...
- **`UpstreamNotJsonError` 分類過寬**可能把真正的解析錯誤誤判為可重試——僅以 content-type 與二進位魔術數字判斷，勿以「json() 拋錯」單一條件涵蓋所有情況
- **07-24 00985A 的缺口不會被本 PR 修復**（野村目前維護中；待維護結束後探測其是否支援 `fetch_at`，若支援即可由歷史回補片一併補齊）。已知下游影響：07-25 該檔的 diff 基準會是 07-23，07-24 發生的異動將被歸到 07-25 的事件日期——此為缺資料下的合理行為，記錄備查，不在本 PR 處理
