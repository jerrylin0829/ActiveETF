...
參考 etfcross.com 的「主被動 ETF 交叉持股」視角：該站把多檔 ETF 的持股疊起來，呈現市場共識持股與產業資金輪動，但只收錄 8 檔主動 ETF（規模前段班、5 家投信）。本專案已全覆蓋 27 檔主動股票型 ETF，`holdings_snapshot` 足以計算同類視圖且覆蓋面更完整。
...
- 分母存進表中是為了資料缺口透明：某日僅 25/27 檔有資料時平均仍正確，且前端可據此顯示黃條
