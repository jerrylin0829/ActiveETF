...
4. **位移是逐檔的，不是逐 adapter 的**。以 `HISTORY_REQUEST_OFFSETS: dict[str, int]` 覆寫，未列出者用模組預設。已知需要覆寫的是統一的全球型 **00988A（+2）**——同一支 adapter 的台股型 00981A／00403A 為 +1。（其餘三檔全球型 00402A、00983A、00990A 實測與同 adapter 的台股型一致，不需覆寫。）
...
- **日期 gate**（2026-07-31 新增）：資料日不等於目標日時不得寫入且計為驗證失敗；**相鄰兩日持股完全相同時仍須擋住**（內容比對在這種情況下沒有鑑別力，只有資料日能擋）；日曆視窗需涵蓋目標範圍前後的交易日；同一份 payload 內多列的資料日不一致時回 `None`（不猜）
