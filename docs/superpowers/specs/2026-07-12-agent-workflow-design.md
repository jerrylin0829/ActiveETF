...
1. **每日運行健康**：確認排程有跑、`scrape_log` 無未補的 `fail`、缺口可補則補（重跑 pipeline 回看 3 天）。
...
- `data-gap`：Operator 角色的 runbook（讀 `scrape_log` → 判斷可否補 → 重跑 pipeline → 核對照組網站），CLAUDE.md 已規劃，與 Operator 角色一體。
...
- **不設獨立 Integration Tester**：E2E 驗證是 Generator 的交付責任 + Evaluator gate 的一條（已在 `pr-review-checklist.md` 要求「涉及 DB/外部依賴需附整合或 smoke 證據」），獨立成 agent 反而讓「誰該跑起來」變模糊。
